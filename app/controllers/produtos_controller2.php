	...
	function frete( $id = null, $strProdutoNome = null ) 
	{
		...
		if( isset( $produto['Opcao'] ) )
		{
			...
			foreach ( $produto['Opcao'] as $opcao )
			{
				...
				if( $opcao['chave'] == 'produto_imagem' && empty( $opcao['valor'] ) ): $produto_imagem_null = "imagem?"; endif;
			...
			if( isset( $this->data ) && !empty($this->data['Produto']['cepdestino']))
			{
				 ...
				 * nova.
				 */
				if( empty($envio_local) && empty($envio_sedex) && empty($envio_sedexacobrar) && empty($envio_pac) )
				{
					$envio_pac = 
					$envio_sedex = 
					$envio_sedexacobrar =
